## Knitdeck

Knitdeck is a small cyberdeck built for reading knitting patterns. It shows a pattern on an e-ink screen, and a few buttons turn the pages and hold your place while you knit.

![TODO::The finished Knitdeck, shown closed and then open with a pattern on its e-ink screen.](images/knitdeck-final.jpg){:width="450px"}

### Inspiration

Knitdeck grew out of a love of making and crafts, and a big pile of knitting patterns. The aim was something simple that felt like it belonged in the craft world rather than the tech one.

This is a figma mood board, of inspiration for the look and feel of it.

![A Figma mood board of knitting and craft references, used to set the look and feel.](images/knitdeck-inspo.png){:width="450px"}

### The enclosure

Knitdeck started from an old 1990s sewing box, bought second-hand — the kind that often turns up in charity shops and thrift stores. It already carried the craft feel the project was reaching for.

![TODO::The second-hand 1990s sewing box used as the enclosure.](images/knitdeck-enclosure.jpg){:width="450px"}

### Making everything fit

Next came working out how everything would fit inside. The first plan was optimistic — the real parts needed far more room than expected. Laying the components out by hand, including the battery and the Raspberry Pi, showed what would actually fit and where each piece could go.

![TODO::The parts laid out in the box to plan where the battery and Raspberry Pi would sit.](images/knitdeck-layout.jpg){:width="450px"}

### Building it

The inside structure is cardboard. It took a lot of making and adjusting — cutting, testing and trimming pieces until everything sat right.

![TODO::Cutting the cardboard base to shape.](images/knitdeck-make-1.gif){:width="450px"}

![TODO::Test-fitting the screen in its cardboard frame.](images/knitdeck-make-2.gif){:width="450px"}

![TODO::Adjusting the supports so the parts sit snugly.](images/knitdeck-make-3.gif){:width="450px"}

### Soldering

Soldering was the fiddliest part, and the one that felt most permanent. A build with no buttons is far simpler — even a few buttons adds a tangle of wires to manage.

![TODO::Soldering the button wires to the Raspberry Pi.](images/knitdeck-solder-1.gif){:width="450px"}

![TODO::The tangle of wires behind the buttons.](images/knitdeck-solder-2.gif){:width="450px"}

### The software

The software came together bit by bit, going back and forth with the code. It is a simple program that shows the pattern pages on the screen and moves through them with the buttons.

![TODO::The Knitdeck software running, showing a pattern on the screen.](images/knitdeck-software.jpg){:width="450px"}

### Making it yours

One of the best parts was decorating it and making it personal.

![TODO::The decorated Knitdeck, with a close-up of the details.](images/knitdeck-decorate.jpg){:width="450px"}
